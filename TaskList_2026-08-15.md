...
**Where you are:** Stages A and B are complete. Migrations 001–011 applied to
production, dev aligned and applied through `dev/013`, v4.36 deployed. The
unauthenticated takeover is closed, both PIN oracles are rate-limited, every
leaked credential is rotated, and the residual `TRUNCATE`/`TRIGGER`/`REFERENCES`
grants are gone. All paperwork is public.

Anonymous reads of names, colours, shift patterns and schedules remain open.
That is the entire remaining exposure, and it needs Stage C then Stage D.

**Next:** Stage C — waiter identity via `signInAnonymously()`.

**Numbering:** one shared counter across both databases from here on; next free
number is `014`. See `supabase/migrations/README.md`.

---

## Every link you need

### Production — Supabase project `osmica-production` (ref `vuvvzzktrxydfxgxugke`)

> ⚠️ Renamed from `ZamijeniMe` on 17 Aug 2026. Display name only — the ref, both
> keys and all data are unchanged, so nothing in `osmica.html` needed redeploying.
>
> ⚠️ `osmica-production` **is** production. Before running anything, confirm the project
...
— confirm top-left says `osmica-production`, paste `supabase/migrations/007_add_token_keyed_set_pin.sql`, Run.
...
-- The link to open. NOTE the explicit osmica.html — the bare directory URL does
-- NOT work: index.html is a <meta http-equiv="refresh"> and a meta refresh drops
-- the query string, so the app boots with no token and lands on the home screen.
select 'https://mpivcevic.github.io/osmica/osmica.html?invite=' || invite_token
...
### ✅ Tidy-up, done 17 Aug 2026

The production Supabase project was renamed `ZamijeniMe` → **`osmica-production`**.
Display name only; ref, keys and data untouched, so nothing needed redeploying.

Deliberately left until the migrations were finished: `ZamijeniMe` and
`osmica-dev` are impossible to confuse at a glance, and picking the wrong project
is the one mistake that actually costs something. `osmica-production` was chosen
over the originally-planned `osmica-prod` for the same reason — it diverges from
`osmica-dev` at the first character after the shared `osmica-`, rather than
looking similar at a glance.

**From here on, the top-left project-name check before running SQL matters more
than it did**, because both names now begin the same way.
